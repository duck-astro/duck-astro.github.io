...
const collisionBoxes = [
  ...
  { minX: 9.99, maxX: 10.08, minZ: -2.08, maxZ: 2.08 },
  { minX: 1.99, maxX: 10.08, minZ: 1.99, maxZ: 2.08 },
...
const tourStops = [
  ...
  },
  {
    ...
    position: new THREE.Vector3(6.45, 1.52, 1.02),
    target: new THREE.Vector3(6.45, 1.0, 1.77),
  },
];
...
async function loadTextures() {
  ...
  const files = {
    ...
    yuiSticker: "yui-switch-sticker.png",
  ...
  manager.onProgress = (_url, loaded, total) => {
    ...
    loadingStatus.textContent = `载入墙面与陈列图片 ${loaded}/${total}`;
  };
...
function buildRoom(textures) {
  ...
  // 新展区改为无缝浅蓝色塑胶地板；不使用纹理和砖缝。
  const extensionFloorMaterial = new THREE.MeshStandardMaterial({
    color: 0xb9dce6,
    roughness: 0.72,
    metalness: 0.01,
  });

  // 两块地板在 x=2.0 处严格相接，避免交界处 z-fighting。
  addBox(scene, [4.0, 0.06, 4.0], [0, -0.03, 0], floorMaterial, false, true);
  addBox(scene, [0.06, 3.04, 4.06], [-2.03, 1.5, 0], wallMaterial, false, true);
  // The former poster wall opens into a new 8 m × 4 m space aligned to the window side.
  ...
  addBox(scene, [8.0, 0.06, 4.0], [6, -0.03, 0], extensionFloorMaterial, false, true);
  addBox(scene, [8.06, 3.04, 0.06], [6, 1.5, -2.03], wallMaterial, false, true);
  addBox(scene, [8.06, 3.04, 0.06], [6, 1.5, 2.03], wallMaterial, false, true);
  addBox(scene, [0.06, 3.04, 4.06], [10.03, 1.5, 0], wallMaterial, false, true);
  addBox(scene, [8.05, 0.05, 4.05], [6, 3.025, 0], ceilingMaterial, false, true);
...
function buildCabinetContents(textures) {
  const frame = addBox(scene, [0.035, 0.39, 0.72], [-1.43, 1.63, -1.25], material(0x33261d, {
    roughness: 0.7,
  }));
  frame.castShadow = true;
  const still = new THREE.Mesh(
    new THREE.PlaneGeometry(0.66, 0.371),
    new THREE.MeshStandardMaterial({ map: textures.koiasFrame, roughness: 0.68 }),
  );
  still.rotation.y = Math.PI / 2;
  still.position.set(-1.407, 1.63, -1.25);
  still.userData.interaction = {
    kind: "image",
    label: "查看《恋爱小行星》截图",
    kicker: "木柜陈列",
    title: "米拉看书",
    url: assetUrl("koias-frame.webp"),
    note: "《恋爱小行星》截图",
  };
  interactables.push(still);
  scene.add(still);
...
function pointLocalXAxis(object, direction) {
...
function createCem40RefractorStationLegacy() {
  const group = new THREE.Group();
...
function createCem40RefractorStation() {
  ...
  const latitude = THREE.MathUtils.degToRad(22);
  const polarAxis = new THREE.Vector3(-Math.cos(latitude), Math.sin(latitude), 0).normalize();
  // DEC 轴与极轴正交。零位时镜筒在 DEC 轴的上侧，重锤杆指向地面。
  const telescopeSide = new THREE.Vector3(Math.sin(latitude), Math.cos(latitude), 0).normalize();
  const counterweightSide = telescopeSide.clone().negate();
  const black = material(0x121820, { metalness: 0.62, roughness: 0.28 });
  const charcoal = material(0x252c33, { metalness: 0.54, roughness: 0.36 });
  const red = material(0xc43b28, { metalness: 0.66, roughness: 0.24 });
  const redDark = material(0x84241d, { metalness: 0.58, roughness: 0.3 });
  const silver = material(0xc7ccce, { metalness: 0.92, roughness: 0.14 });
  ...
  // 低纬度底座：极轴相对水平面抬升 22°，两侧红色 C 形支架围住 RA 轴。
  const raSouth = new THREE.Vector3(0.055, 1.12, 0);
  // CEM40 的极轴镜壳体只保留紧凑的主体长度；极轴仰角和轴线方向不变。
  const polarHousingLength = 0.22;
  const raNorth = raSouth.clone().addScaledVector(polarAxis, polarHousingLength);
  for (const z of [-0.075, 0.075]) {
    addBoxBetween(group, [0.06, 1.105, z], [raNorth.x + 0.04, raNorth.y - 0.02, z], 0.065, 0.055, red);
    addBoxBetween(group, [-0.035, 1.12, z], [raSouth.x - 0.055, raSouth.y + 0.08, z], 0.055, 0.055, redDark);
  }
  addBox(group, [0.2, 0.13, 0.19], [-0.015, 1.16, 0], black);
  addCylinderBetween(group, raSouth.toArray(), raNorth.toArray(), 0.092, black, 32);
  addCylinderBetween(
    group,
    raSouth.clone().addScaledVector(polarAxis, 0.03).toArray(),
    raNorth.clone().addScaledVector(polarAxis, 0.035).toArray(),
    0.072,
    red,
    30,
  );

  const decCenter = raNorth.clone().addScaledVector(polarAxis, 0.035);
  addCylinderBetween(
    group,
    decCenter.clone().addScaledVector(telescopeSide, -0.125).toArray(),
    decCenter.clone().addScaledVector(telescopeSide, 0.145).toArray(),
    0.082,
    black,
    30,
  );
  const decMotor = decCenter.clone().addScaledVector(telescopeSide, -0.08).add(new THREE.Vector3(0, 0, -0.09));
  const motorHousing = new THREE.Mesh(createRoundedBoxGeometry(0.15, 0.15, 0.12, 0.018), charcoal);
  motorHousing.position.copy(decMotor);
  motorHousing.castShadow = true;
  group.add(motorHousing);

  const saddlePosition = decCenter.clone().addScaledVector(telescopeSide, 0.18);
  const saddle = addBox(group, [0.27, 0.052, 0.135], saddlePosition.toArray(), black);
  pointLocalXAxis(saddle, polarAxis);
  const saddleInset = addBox(group, [0.21, 0.018, 0.115], saddlePosition.clone().addScaledVector(telescopeSide, 0.035).toArray(), silver);
  pointLocalXAxis(saddleInset, polarAxis);
  addCylinderBetween(
    group,
    saddlePosition.clone().add(new THREE.Vector3(0, 0, -0.105)).toArray(),
    saddlePosition.clone().add(new THREE.Vector3(0, 0, -0.16)).toArray(),
    0.018,
    redDark,
    18,
  );

  // 零位：重锤杆沿 DEC 轴向下，镜筒在赤道仪上方并与极轴平行指向北天极。
  const shaftStart = decCenter.clone().addScaledVector(counterweightSide, 0.08);
  const shaftEnd = shaftStart.clone().addScaledVector(counterweightSide, 0.36);
  const shaft = addCylinderBetween(group, shaftStart.toArray(), shaftEnd.toArray(), 0.014, silver, 18);
  const weightPosition = shaftStart.clone().lerp(shaftEnd, 0.66);
  const weight = new THREE.Mesh(new THREE.CylinderGeometry(0.092, 0.092, 0.07, 32), black);
  weight.position.copy(weightPosition);
  weight.quaternion.copy(shaft.quaternion);
  weight.castShadow = true;
  group.add(weight);
  const safetyEnd = shaftEnd.clone().addScaledVector(counterweightSide, 0.035);
  addCylinderBetween(group, shaftEnd.toArray(), safetyEnd.toArray(), 0.021, black, 18);

  const tubeAssembly = new THREE.Group();
  tubeAssembly.position.copy(saddlePosition.clone().addScaledVector(telescopeSide, 0.075));
  pointLocalXAxis(tubeAssembly, polarAxis);
  group.add(tubeAssembly);
  const tubeRadius = 0.052;
  addAxialCylinder(tubeAssembly, tubeRadius, tubeRadius, 0.42, [0.015, 0, 0], carbon, "x", 40);
  // 物镜端朝向 +polarAxis（北天极）；原先前后端相反，现将物镜和调焦端互换。
  addAxialCylinder(tubeAssembly, 0.069, 0.069, 0.17, [0.265, 0, 0], black, "x", 40);
  addAxialCylinder(tubeAssembly, 0.061, 0.061, 0.025, [0.36, 0, 0], charcoal, "x", 36);
  addAxialCylinder(tubeAssembly, 0.047, 0.047, 0.009, [0.378, 0, 0], lens, "x", 34);
  for (const tubeX of [-0.13, 0.12]) {
    const ring = new THREE.Mesh(new THREE.TorusGeometry(0.061, 0.011, 10, 34), black);
    ring.rotation.y = Math.PI / 2;
    ring.position.x = tubeX;
    ring.castShadow = true;
    tubeAssembly.add(ring);
  }
  addBox(tubeAssembly, [0.36, 0.024, 0.054], [0, -0.073, 0], silver);
  addAxialCylinder(tubeAssembly, 0.052, 0.043, 0.095, [-0.26, 0, 0], black, "x", 30);
  addAxialCylinder(tubeAssembly, 0.043, 0.032, 0.075, [-0.345, 0, 0], charcoal, "x", 28);
  addAxialCylinder(tubeAssembly, 0.018, 0.018, 0.155, [-0.02, 0.104, 0], black, "x", 18);
  addBox(tubeAssembly, [0.15, 0.02, 0.028], [-0.02, 0.076, 0], black);
  addBox(tubeAssembly, [0.14, 0.022, 0.007], [0.02, 0, 0.054], redDark, false, false);

  const latitudePlate = addBox(group, [0.11, 0.075, 0.012], [0.06, 1.17, -0.108], charcoal, false, false);
  latitudePlate.rotation.z = latitude;
  for (let i = 0; i < 4; i += 1) {
    addAxialCylinder(group, 0.006, 0.006, 0.009, [0.025 + i * 0.018, 1.18 + i * 0.007, -0.118], silver, "z", 10);
  }
  addAxialCylinder(group, 0.025, 0.025, 0.035, [-0.08, 1.17, -0.105], redDark, "z", 20);
...
function createSolarMaxStation() {
  ...
  createOpenCarryCase(group, { width: 1.24, depth: 0.43, shellColor: 0x171b20 });
...
function createNewtonianDisplayLegacy() {
  const group = new THREE.Group();
...
function createNewtonianDisplay() {
  ...
  const telescopeSide = new THREE.Vector3(Math.sin(latitude), Math.cos(latitude), 0).normalize();
  const counterweightSide = telescopeSide.clone().negate();
  const black = material(0x171c22, { metalness: 0.54, roughness: 0.32 });
  const dark = material(0x2d343b, { metalness: 0.48, roughness: 0.38 });
  const white = material(0xe7e9e7, { metalness: 0.16, roughness: 0.42 });
  const green = material(0x4b9f4d, { metalness: 0.4, roughness: 0.34 });
  const silver = material(0xc4c9cb, { metalness: 0.92, roughness: 0.15 });
  const glass = material(0x0b1218, { metalness: 0.18, roughness: 0.08 });
  ...
  addCylinder(group, 0.105, 0.115, 0.075, [0, 0.84, 0], black, 30);
  addBox(group, [0.18, 0.14, 0.17], [0.035, 0.9, 0], dark);

  // EQ3 的纬度座托起 RA 轴；RA 与 DEC 两轴互相垂直，均保留设置环与锁紧结构。
  const raSouth = new THREE.Vector3(0.085, 0.88, 0);
  // EQ3 的极轴镜壳体按紧凑的实际外形缩短，保留 22° 极轴方向。
  const polarHousingLength = 0.19;
  const raNorth = raSouth.clone().addScaledVector(polarAxis, polarHousingLength);
  addBoxBetween(group, [0.07, 0.87, -0.065], [raNorth.x + 0.025, raNorth.y - 0.015, -0.065], 0.065, 0.055, black);
  addBoxBetween(group, [0.07, 0.87, 0.065], [raNorth.x + 0.025, raNorth.y - 0.015, 0.065], 0.065, 0.055, black);
  addCylinderBetween(group, raSouth.toArray(), raNorth.toArray(), 0.072, black, 30);

  const raRingCenter = raSouth.clone().lerp(raNorth, 0.55);
  const raRing = new THREE.Mesh(new THREE.TorusGeometry(0.078, 0.008, 8, 32), silver);
  raRing.position.copy(raRingCenter);
  raRing.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), polarAxis);
  group.add(raRing);

  const decCenter = raNorth.clone().addScaledVector(polarAxis, 0.025);
  addCylinderBetween(
    group,
    decCenter.clone().addScaledVector(telescopeSide, -0.12).toArray(),
    decCenter.clone().addScaledVector(telescopeSide, 0.14).toArray(),
    0.065,
    dark,
    28,
  );
  const decSettingCenter = decCenter.clone().addScaledVector(telescopeSide, 0.055);
  const decRing = new THREE.Mesh(new THREE.TorusGeometry(0.071, 0.007, 8, 30), silver);
  decRing.position.copy(decSettingCenter);
  decRing.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), telescopeSide);
  group.add(decRing);

  const shaftStart = decCenter.clone().addScaledVector(counterweightSide, 0.085);
  const shaftEnd = shaftStart.clone().addScaledVector(counterweightSide, 0.39);
  const shaft = addCylinderBetween(group, shaftStart.toArray(), shaftEnd.toArray(), 0.012, silver, 18);
  for (const fraction of [0.48, 0.72]) {
    const weight = new THREE.Mesh(new THREE.CylinderGeometry(0.076, 0.076, 0.055, 30), black);
    weight.position.copy(shaftStart.clone().lerp(shaftEnd, fraction));
    weight.quaternion.copy(shaft.quaternion);
    weight.castShadow = true;
    group.add(weight);
  }
  const shaftCap = shaftEnd.clone().addScaledVector(counterweightSide, 0.03);
  addCylinderBetween(group, shaftEnd.toArray(), shaftCap.toArray(), 0.018, black, 16);

  const saddlePosition = decCenter.clone().addScaledVector(telescopeSide, 0.17);
  const saddle = addBox(group, [0.25, 0.045, 0.125], saddlePosition.toArray(), black);
  pointLocalXAxis(saddle, polarAxis);
  const dovetail = addBox(group, [0.42, 0.022, 0.075], saddlePosition.clone().addScaledVector(telescopeSide, 0.04).toArray(), green);
  pointLocalXAxis(dovetail, polarAxis);
  addAxialCylinder(group, 0.024, 0.024, 0.04, saddlePosition.clone().add(new THREE.Vector3(0, 0, -0.105)).toArray(), dark, "z", 18);

  const tubeAssembly = new THREE.Group();
  tubeAssembly.position.copy(saddlePosition.clone().addScaledVector(telescopeSide, 0.105));
  pointLocalXAxis(tubeAssembly, polarAxis);
  group.add(tubeAssembly);
  const length = 0.69;
  const radius = 0.091;
  addAxialCylinder(tubeAssembly, radius, radius, length, [0, 0, 0], black, "x", 40);
  addAxialCylinder(tubeAssembly, 0.099, 0.099, 0.045, [-0.325, 0, 0], white, "x", 36);
  addAxialCylinder(tubeAssembly, 0.099, 0.099, 0.045, [0.325, 0, 0], white, "x", 36);
  addAxialCylinder(tubeAssembly, 0.087, 0.087, 0.009, [-0.352, 0, 0], glass, "x", 34);
  for (const tubeX of [-0.18, 0.16]) {
    const ring = new THREE.Mesh(new THREE.TorusGeometry(0.102, 0.011, 10, 34), white);
    ring.rotation.y = Math.PI / 2;
    ring.position.x = tubeX;
    tubeAssembly.add(ring);
  }
  ...
  addBox(tubeAssembly, [0.15, 0.018, 0.025], [-0.08, 0.11, 0], white);

  // 两轴蜗杆慢动作杆与大号锁紧旋钮是 EQ3 的显著外形特征。
  const raCableStart = raSouth.clone().lerp(raNorth, 0.42).add(new THREE.Vector3(0, 0, 0.08));
  const raCableEnd = raCableStart.clone().add(new THREE.Vector3(0.16, -0.16, 0.09));
  addCylinderBetween(group, raCableStart.toArray(), raCableEnd.toArray(), 0.006, black, 12);
  addAxialCylinder(group, 0.022, 0.022, 0.04, raCableEnd.toArray(), black, "z", 18);
  const decCableStart = decCenter.clone().add(new THREE.Vector3(0, 0, -0.075));
  const decCableEnd = decCableStart.clone().add(new THREE.Vector3(-0.12, -0.13, -0.08));
  addCylinderBetween(group, decCableStart.toArray(), decCableEnd.toArray(), 0.006, black, 12);
...
function createFlatFieldPanel() {
  const group = new THREE.Group();
  group.position.set(8.37, 0, 1.96);
  group.rotation.y = Math.PI;
  scene.add(group);
  const frame = material(0x080a0d, { metalness: 0.46, roughness: 0.4 });
  addBox(group, [0.64, 0.46, 0.025], [0, 0.31, 0], frame);
  addBox(group, [0.58, 0.4, 0.012], [0, 0.31, -0.02], material(0xffffff, {
    emissive: 0xffffff,
...
function buildDryCabinet() {
  const group = new THREE.Group();
  group.position.set(6.45, 0, 1.77);
  scene.add(group);
  const unitWidth = 1.3;
  const unitGap = 0.1;
  const width = unitWidth * 2 + unitGap;
  const height = 1.78;
  const depth = 0.46;
  ...
  const unitCenters = [-(unitWidth + unitGap) / 2, (unitWidth + unitGap) / 2];
  unitCenters.forEach((centerX, index) => {
    addBox(group, [unitWidth, 0.08, depth], [centerX, 0.04, 0], frame);
    addBox(group, [unitWidth, 0.08, depth], [centerX, height - 0.04, 0], frame);
    addBox(group, [0.07, height, depth], [centerX - unitWidth / 2 + 0.035, height / 2, 0], frame);
    addBox(group, [0.07, height, depth], [centerX + unitWidth / 2 - 0.035, height / 2, 0], frame);
    addBox(group, [unitWidth - 0.12, height - 0.12, 0.025], [centerX, height / 2, depth / 2 - 0.012], inside);
    for (const y of [0.39, 0.71, 1.03, 1.35]) {
      addBox(group, [unitWidth - 0.12, 0.025, depth - 0.08], [centerX, y, 0], inside, true, true);
      addBox(
        group,
        [unitWidth - 0.16, 0.012, 0.025],
        [centerX, y + 0.025, -depth / 2 + 0.035],
        material(0xc7e8f4, { emissive: 0x8ad8f2, emissiveIntensity: 0.7, roughness: 0.24 }),
        false,
        false,
      );
    }
    // 每个防潮柜改为左右双开门：两扇门分别从外侧铰链向中间打开。
    const doorWidth = (unitWidth - 0.04) / 2;
  ...
  createShelfLabel(group, "CAMERAS", -1.02, 1.37, 0.3);
  createShelfLabel(group, "EYEPIECES", -0.98, 1.05, 0.36);
  createShelfLabel(group, "CONTROLLERS & OPTICS", -0.78, 0.73, 0.5);
  createShelfLabel(group, "ADAPTERS & FILTERS", -0.8, 0.41, 0.5);
...
function createShelfLabel(parent, textValue, x, y, width) {
  ...
  label.position.set(x, y + 0.04, 0.208);
...
function setCameraAtStop(index, immediate = false) {
  ...
  if (activeStop === 1) showTransientHint("点击柜门，或查看相框与社刊封面", 2600);
